...
def uiteindelijke_waarden(variabelen, aantaluren, namen_apparaten, wattagelijst, huidig_batterijniveau, verliesfactor,
                          winstfactor, huidige_temperatuur, batterij_ontladen, batterij_opladen, warmtepomp):
    print('-' * 30)
    ...
    nieuwe_temperatuur = pe.value(
        huidige_temperatuur + warmtepomp[1] - verliesfactor[0])
...
def voorwaarden_warmteboiler(warmtepomp,voorwaardenlijst, warmteverliesfactor, warmtewinst,
    ...
    if aanvankelijke_temperatuur < ondergrens:
        voorwaardenlijst.add(expr=warmtepomp[1] == warmtewinst[0])
    elif aanvankelijke_temperatuur > bovengrens:
        voorwaardenlijst.add(expr=warmtepomp[1] == 0)
    else:

        for p in range(1, aantaluren+1):
            temperatuur_dit_uur = temperatuur_dit_uur - warmteverliesfactor[p-1] + warmtepomp[p]
...
from parameters_test import wattage_warmtemomp as wattage_warmtepomp
#######################################################################################################
...
m.voorwaarden_warmtepomp_grenzen = pe.ConstraintList()
for p in range(1, aantal_uren+1):
    m.voorwaarden_warmtepomp_grenzen.add(expr= (0, m.warmtepomp[p], temperatuurwinst_per_uur[p-1]))
...
voorwaarden_warmteboiler(m.warmtepomp, m.voorwaarden_warmtepomp, verliesfactor_huis_per_uur,
...
kost, apparaten_aanofuit, nieuw_batterijniveau, nieuwe_temperatuur, pos_of_neg_opladen= uiteindelijke_waarden(m.apparaten, aantal_uren,
                                                                                           ...
                                                                                           m.batterij_opladen, m.warmtepomp)
